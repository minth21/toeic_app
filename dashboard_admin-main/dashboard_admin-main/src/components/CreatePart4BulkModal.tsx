...
interface CreatePart4BulkModalProps {
    open: boolean;
    onCancel: () => void;
    onSuccess: () => void;
    partId: string | null;
...
export default function CreatePart4BulkModal({ open, onCancel, onSuccess, partId, currentAudioUrl, partNumber, initialData }: CreatePart4BulkModalProps) {
    ...
    const handleAddGroup = () => {
        ...
        const nextStartQ = lastGroup ? lastGroup.questions[2].questionNumber + 1 : 71;
        const newGroup: GroupData = {
            transcript: '', graphicPreview: null, graphicFile: null, translationSentences: [], keyVocabulary: [],
            questions: Array.from({ length: 3 }, (_, qIdx) => ({
                questionNumber: nextStartQ + qIdx, questionText: '', optionA: '', optionB: '', optionC: '', optionD: '', correctAnswer: 'A'
            }))
        };
    ...
    const handleDownloadTemplate = () => {
        ...
        XLSX.utils.book_append_sheet(wb, ws, `Part4`);
        XLSX.writeFile(wb, `Part4_Bulk_Template.xlsx`);
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            for (const group of validGroups) {
                ...
                // Backend bắt buộc danh sách câu hỏi không được rỗng
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
                    {groups.map((group, gIdx) => (
                        <Card 
                            ...
                            title={
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                    <Space>
                                        ...
                                        <Text strong style={{ color: '#1E3A8A' }}>CÂU {group.questions[0].questionNumber} - {group.questions[2]?.questionNumber}</Text>
                                    </Space>
                            ...
                            style={{ borderRadius: 16, border: 'none', boxShadow: '0 10px 30px -5px rgba(37, 99, 235, 0.1)' }}
                        >
                            <Row gutter={24}>
                                <Col span={10}>
                                    <div style={{ background: '#F0F7FF', padding: 16, borderRadius: 12, border: '1px solid #BFDBFE', height: '100%' }}>
                                        <div style={{ marginBottom: 16 }}>
                                            <div style={{ fontWeight: 800, fontSize: 12, color: '#475569', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                                                <FileTextOutlined /> TRANSCRIPT BÀI NÓI
                                            </div>
                                            <ReactQuill 
                                                theme="snow" 
                                ...
                                <Col span={14}>
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        {group.questions.map((q, qIdx) => (
                                            <div key={qIdx} style={{ background: '#fff', padding: '12px 16px', borderRadius: 12, border: '1px solid #F1F5F9' }}>
                                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                                                    <Space>
                                                        <span style={{ fontWeight: 800, color: '#1E293B' }}>{q.questionNumber}.</span>
                                                        <Input variant="borderless" placeholder="Nội dung câu hỏi..." value={q.questionText} onChange={(e) => handleQuestionChange(gIdx, qIdx, 'questionText', e.target.value)} style={{ fontWeight: 600, width: 420 }} />
                                                    </Space>
                                                    <Space>
                                                        <Select size="small" value={q.correctAnswer} onChange={(val) => handleQuestionChange(gIdx, qIdx, 'correctAnswer', val)} style={{ width: 60 }} styles={{ popup: { root: { minWidth: 60 } } }}>
                                                    ...
                                                    {['A', 'B', 'C', 'D'].map(opt => (
                                                        <Col span={12} key={opt}>
                                                            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                                                ...
                                                                <Input size="small" variant="borderless" value={(q as any)[`option${opt}`]} onChange={(e) => handleQuestionChange(gIdx, qIdx, `option${opt}` as any, e.target.value)} style={{ fontSize: 13 }} />
                                                            </div>
                                                        </Col>
                                                    ))}
                                                </Row>

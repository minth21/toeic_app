 ...
 * Based on the official tables provided by the user.
 */
...
/**
 * [CRITICAL UPDATE] REMOVED SCALING LOGIC
 * Following Rule #7: We sum the raw correct counts of all parts and then convert.
 * This function now returns the TOEIC score for the ABSOLUTE correct count provided.
 */
export const calculateRawToeicScore = (correct: number, type: 'LISTENING' | 'READING'): number => {
    return type === 'LISTENING' ? getListeningScore(correct) : getReadingScore(correct);
};

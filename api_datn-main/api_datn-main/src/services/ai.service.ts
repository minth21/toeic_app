...
5. **GIỮ NGUYÊN BỐ CỤC (CHỈ cho trường passageHtml):**
   ...
   - Các dạng đặc biệt như Email, Advert, Memo, Article, Letter, Review PHẢI được trình bày xuống dòng và ngắt đoạn đúng như ảnh gốc.

6. DỊCH SONG NGỮ (BẮT BUỘC TÁCH CÂU - SENTENCE SPLITTING):
   - Nhiệm vụ quan trọng nhất của bạn là PHÂN TÁCH văn bản thành từng câu nhỏ (Sentence-by-sentence) hoặc từng dòng (Line-by-line). 
   - MỖI CÂU phải là một đối tượng riêng biệt trong mảng 'items'.
   - TUYỆT ĐỐI CẤM gộp nhiều câu vào cùng một trường 'en' hoặc 'vi'.
   - Các nội dung trong passageTranslationData PHẢI là VĂN BẢN THUẦN (Plain Text), TUYỆT ĐỐI KHÔNG chứa thẻ HTML.
...
export const evaluateProgress = async (
    ...
    try {
        ...
        const result = await executeAITaskWithRetry({
            contents: [{ role: 'user', parts: [{ text: prompt }] }],
            generationConfig: {
                maxOutputTokens: isPart34 ? 600 : 1000, 
                temperature: 0.4,
                responseMimeType: "application/json",
            }
        });
...
export const scanPart6FromImageService = async (imageBuffer: Buffer, mimeType: string) => {
    const prompt = `${MASTER_PROMPT_RULES}\nBạn là trợ lý nhập liệu TOEIC Part 6 chuyên nghiệp.
    NHIỆM VỤ: Trích xuất passage, questions, và dịch song ngữ chi tiết.
    ...
    Output JSON: 
    { 
        "passageHtml": "Nội dung đoạn văn (giữ nguyên layout, ngắt dòng bằng <p>, <br/>, <b>). LƯU Ý: Tuyệt đối không đưa tiêu đề câu hỏi (như 'Questions 131-134 refer to...') vào đây.", 
        "passageTranslationData": [{"type": "passage", "label": "[Loại văn bản: VD Letter]", "items": [{"id": "s1", "en": "...", "vi": "...", "vocab": [{"text": "...", "type": "n/v/adj/adv", "meaning": "...", "ipa": "/.../"}]}]}], 
...
export const scanPart7FromImageService = async (imageBuffer: Buffer, mimeType: string) => {
    const prompt = `${MASTER_PROMPT_RULES}\nBạn là trợ lý nhập liệu TOEIC Part 7 chuyên nghiệp.
    NHIỆM VỤ: Trích xuất passage, questions, suy luận đáp án và dịch song ngữ chi tiết. 
    
    YÊU CẦU QUAN TRỌNG (BẮT BUỘC):
    1. PHÂN LOẠI & DỊCH TOÀN BỘ: Nhận diện chính xác loại văn bản (Email, Advert, Article, Schedule, Web Page, Letter, Review, v.v.). Dịch TẤT CẢ văn bản trong ảnh bao gồm cả TIÊU ĐỀ, headers, footnotes.
    2. TÁCH BIỆT TỪNG CÂU (SENTENCE-BY-SENTENCE): Trong mảng "items", hãy PHÂN TÁCH văn bản thành từng câu ngắn. MỖI CÂU LÀ MỘT ITEM. Tuyệt đối không gộp 2 câu vào 1 item. Nếu là BẢNG (Schedule/Invoice), mỗi dòng của bảng phải là một item.
    3. TỪ VỰNG: Trích xuất TỐI THIỂU 10 từ vựng trọng tâm.
...
const scanStructureAndQuestions = async (
    ...
    1. PHÂN LOẠI PASSAGES: Với mỗi văn bản xuất hiện, bạn phải xác định chính xác nó là loại gì (Email, Article, Advertisement, Web Page, Memo, Schedule, Review, Message Chain, Letter, v.v.).
...
const enrichQuestionsWithTranslations = async (
    ...
    const prompt = `${MASTER_PROMPT_RULES}\nBạn là chuyên gia luyện thi TOEIC 990. Nhiệm vụ cực kỳ quan trọng: Dịch và cung cấp PHÂN TÍCH & BẰNG CHỨNG GIẢI THÍCH CHI TIẾT cho các câu hỏi Part 7 sau đây.
    
    YÊU CẦU BẮT BUỘC:
    1. TỪ VỰNG TỔNG (vocabulary): Phải trích xuất TỐI THIỂU 10 từ vựng quan trọng (hay/khó) xuất hiện trong bài đọc và câu hỏi, định nghĩa rõ ràng. 
    2. DỊCH & PHÂN TÍCH: Dịch chuẩn xác câu hỏi, options. Analysis tối thiểu 30 chữ. Evidence trích dẫn chuẩn kèm dịch.
    
    Output JSON: 
    { 
        "translations": [ { "questionNumber": 147, "questionTranslation": "...", "optionTranslations": {"A": "...", "B": "...", "C": "...", "D": "..."}, "analysis": "...", "evidence": "..." } ], 
        "vocab": [ { "text": "...", "lemma": "...", "type": "n/v/adj/adv", "meaning": "...", "ipa": "/.../" } ] 
    }`;
...
const extractPassageDetailService = async (
    images: { buffer: Buffer; mimeType: string }[],
    passageLabel: string
): Promise<{ type: string; label: string; passageHtml: string; passageTranslationData: any[]; items: any[]; keyVocabulary: any[] }> => {
    const prompt = `${MASTER_PROMPT_RULES}
Trích xuất nội dung văn bản cực kỳ chi tiết dựa trên nhãn "${passageLabel}". 

### QUY TẮC CỰC KỲ QUAN TRỌNG (BẮT BUỘC):
1. TÁCH BIỆT TỪNG CÂU (SENTENCE-BY-SENTENCE): Phải chia nhỏ văn bản thành từng CÂU đơn lẻ. Tuyệt đối không được gộp 2-3 câu vào chung một item. Mỗi câu phải có bản dịch tương ứng.
2. PHÂN TÁCH DÒNG: Đối với Header, ngày tháng, chữ ký, mỗi dòng phải là một item.
3. TỪ VỰNG: Trích xuất TỐI THIỂU 10 từ vựng trọng tâm từ đoạn văn này.
...
export const magicScanPart7FromImagesService = async (
    ...
    const [passageResults, enrichmentResult] = await Promise.all([Promise.all(passagePromises), enrichmentPromise]);
    
    const flattenedTranslationData = passageResults.flatMap(pt => pt.passageTranslationData || pt.items || []);

    const mergedVocabMap = new Map();
    (enrichmentResult.vocabulary || []).forEach((v: any) => v.word && mergedVocabMap.set(v.word, v));
    passageResults.forEach(pt => pt.keyVocabulary?.forEach((v: any) => v.word && mergedVocabMap.set(v.word, v)));

    const passageHtml = passageResults.map(pt => pt.passageHtml || pt.items?.map((s: any) => s.en).join(' ') || '').join('\n\n');

    const passageData = passageResults.map((pt, index) => {
        ...
        return {
            ...
            translation: pt.passageTranslationData?.[0]?.items || pt.items || []
        };
    });

    return { 
        passageHtml, 
        passageTranslationData: flattenedTranslationData, 
        passageData, 
...
export const generatePart7ExplanationService = async (
    ...
    LƯU Ý 1: Phải xác định loại đoạn văn (Email, Article, Invoice...) cho từng đoạn trong "passageTranslationData".
    ...
    Output JSON THUẦN TÚY: { "passageTranslationData": [{"type": "passage", "label": "Passage 1: [Loại]", "items": [{"id": "s1", "en": "...", "vi": "...", "vocab": [{"text": "...", "type": "n/v/adj/adv", "meaning": "...", "ipa": "/.../"}]}]}], "vocabulary": [{"word": "...", "type": "n/v/adj/adv", "ipa": "/.../", "meaning": "..."}], "questions": [{"questionNumber": 147, "analysis": "...", "evidence": "...", "optionTranslations": {}}] }`;
...
const scanPart6Structure = async (images: { buffer: Buffer; mimeType: string }[]) => {
    ...
    4. TRÍCH XUẤT ĐOẠN VĂN: Lấy toàn bộ nội dung đoạn văn (Passage). LƯU Ý: Tuyệt đối không đưa tiêu đề câu hỏi (như 'Questions 131-134 refer to...') vào đây.
...
const enrichPart6WithInsights = async (passage: string, questions: any[]) => {
    ...
    YÊU CẦU BẮT BUỘC CHO TƯNG MỖI CÂU:
...
export const magicScanPart6FromImagesService = async (images: { buffer: Buffer; mimeType: string }[]) => {
    ...
    return { 
        ...
        passageTranslationData: insights.passageTranslationData || insights.passageTranslations || [], 
    ...
    *Mỗi giai đoạn phải có: Tên, Thời gian, Chủ điểm trọng tâm, và Lời khuyên "xương máu".*
    ...
    SCHEMA JSON:
    {
      ...
      "currentAnalysis": {
         "strengths": ["...", "..."],
         "weaknesses": ["...", "..."],
         "partPerformance": [ { "part": 1, "status": "Strong/Weak", "comment": "..." } ]
      },
      "phases": [
         { "name": "...", "duration": "...", "focusTopics": ["...", "..."], "expertTip": "..." }
      ],
